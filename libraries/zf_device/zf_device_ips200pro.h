...
* 修改记录
* 日期              作者                备注
* 2025-05-14       pudding            first version (CYT2BL3)
* 2025-07-12       移植适配           移植到 MM32F327X-G8P 平台
********************************************************************************************************************/
/********************************************************************************************************************
* 接线定义：
*                  ------------------------------------
...
#define IPS200PRO_SPI_INDEX            ( SPI_1              )   // 硬件 SPI 号 与普通 IPS200 共用 SPI1
#define IPS200PRO_CLK_PIN              ( SPI1_SCK_A5        )   // 硬件 SPI SCK 引脚 对应普通 IPS200 的 SCK
#define IPS200PRO_MOSI_PIN             ( SPI1_MOSI_A7       )   // 硬件 SPI MOSI 引脚 对应普通 IPS200 的 SDA
#define IPS200PRO_MISO_PIN             ( SPI_MISO_NULL      )   // 硬件 SPI MISO 引脚 无 MISO 引脚
#define IPS200PRO_RST_PIN              ( A6                 )   // 液晶复位引脚 对应普通 IPS200 的 RST
#define IPS200PRO_INT_PIN              ( D0                 )   // 液晶中断/应答引脚 对应普通 IPS200 的 DC
#define IPS200PRO_CS_PIN               ( A4                 )   // CS 片选引脚 对应普通 IPS200 的 CS
...
// 参数说明     dir             亮度1-255
// 返回参数     uint8           状态 0：设置成功  1：设置失败
// 使用示例     ips200pro_set_direction(IPS200PRO_CROSSWISE);  // 设置为横屏显示
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     width           组件的宽度
// 参数说明     height          组件的高度
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
...
// 使用示例     ips200pro_label_printf(label_id, "test=%d", test);
//-------------------------------------------------------------------------------------------------------------------
...
// 返回参数     uint8           状态 0：成功  1：失败
// 使用示例     ips200pro_label_mode(label_id, LABEL_SCROLL);
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     ...             可变参数列表，方法与printf相同
// 返回参数     uint8           状态 0：成功  1：失败
// 使用示例     ips200pro_table_cell_printf(table_id, 1, 1, "test=%d", test);  // 格式化字符串，并显示在表格的第一行 第一列（即左上角的单元格）
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     style           仪表样式
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
// 使用示例     uint16 meter_id = ips200pro_meter_create(0, 0, 60, METER_ANGLE);  // 创建角度指示器
//-------------------------------------------------------------------------------------------------------------------
...
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
// 使用示例     uint16 clock_id = ips200pro_clock_create(0, 0, 150, CLOCK_ANALOG);  // 创建指针时钟
...
// 参数说明     height          组件的高度
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
// 使用示例     uint16 bar_id = ips200pro_progress_bar_create(0, 0, 60, 10);  //
...
// 参数说明     height          组件的高度
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
// 使用示例     uint16 cal_id = ips200pro_calendar_create(0, 0, 200, 200);  //
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     height          组件的高度
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
// 使用示例     uint16 wave_id = ips200pro_waveform_create(0, 0, 200, 200);  //
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     color           点颜色
// 返回参数     uint8           状态 0：成功  1：失败
// 使用示例     ips200pro_waveform_add_value(waveform_id, 1, &point[0], 10, RGB565_RED);  //
...
// 返回参数     uint8           状态 0：成功  1：失败
// 使用示例     ips200pro_waveform_line_state(waveform_id, 1, 0);  // 隐藏线条1
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     height          组件的高度
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
// 使用示例     uint16 img_id = ips200pro_image_create(0, 0, 188, 120);  //
//-------------------------------------------------------------------------------------------------------------------
...
// 参数说明     data_type       线条的数据类型 8位类型与16位类型
// 参数说明     color           线条颜色
// 返回参数     uint8           状态 0：成功  1：失败
...
// 参数说明     height          组件的高度
// 返回参数     uint16          状态 0：创建失败  其他：创建成功，同时返回的数值表示此组件ID
// 使用示例     uint16 con_id = ips200pro_container_create(0, 0, 60, 60);  //
//-------------------------------------------------------------------------------------------------------------------
...
// 注意事项     如果容器宽度高度相同，圆角半径是高度的一半，则最终容器为圆形
//-------------------------------------------------------------------------------------------------------------------
